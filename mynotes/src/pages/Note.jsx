...
function Note({match, history}) {
    ...
    let getNote = async() =>{
        ...
        let response = await fetch(`http://localhost:8000/api/details/${noteId}`)
    ...
    let updateNote = async () => {
        await fetch(`http://localhost:8000/api/update/${noteId}`, {
            method : 'PUT',
            headers: {
    ...
    let deleteNote = async() => {
        await fetch(`http://localhost:8000/api/delete/${noteId}`, {
            method : 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
